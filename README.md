...
graph TD
    subgraph Browser["ブラウザ (React 19 + TypeScript)"]
        UI["🎤 マイク録音 / 📁 ファイルUL<br/>🎶 楽曲検索 / ❤️ お気に入り"]
    end

    Browser -->|HTTPS REST API| Backend

    subgraph Backend["FastAPI バックエンドサーバー"]
        Auth["🔐 認証 (Supabase JWT)"]
        Analyze["📊 音声分析"]
        SongAPI["🎵 楽曲検索 API"]
        Profile["👤 プロフィール管理"]
    end
    ...
    subgraph Pipeline["音声処理パイプライン"]
        direction TB
        NR["ノイズ除去"] --> VS["ボーカル分離 (Demucs)"]
        VS --> PD["ピッチ検出 (CREPE)"]
        PD --> SA["スペクトル分析"]
        SA --> RC["地声/裏声分類"]
    end
    ...
    subgraph DB["データベース"]
        Supa["Supabase<br/>(認証・ユーザー)"]
        SQLite["SQLite<br/>(楽曲情報 5000曲 / 約850アーティスト)"]
    end
```
